...
import { PostCreateRequest, PostResponse } from "@/app/types/post"; // Importação correta
import { ErrorResponse } from "@/app/types/auth"; // Importar ErrorResponse de auth.ts

import { UserAuthResponse } from "@/app/types/auth"; // Para o userId do useSelector (já existente)
...
export default function FeedPage() {
  ...
  const handlePostSubmit = async (event: React.FormEvent) => {
    ...
    try {
        await createPost({ userId: currentUserId, data: { content: newPostContent } }).unwrap();
        setNewPostContent("");
        refetchPosts();
    } catch (err: any) {
