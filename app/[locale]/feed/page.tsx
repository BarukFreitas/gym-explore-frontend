...
export default function FeedPage() {
    ...
    const [createPost, { isLoading: isCreatingPost, isSuccess: createPostSuccess, data: createPostResponseData }] = useCreatePostMutation();
    ...
    useEffect(() => {
        ...
        if (createPostSuccess) {
            ...
            if (createPostResponseData?.pointsAwarded) {
                ...
                // Opcional: dê um feedback genérico se o post foi criado mas sem pontos
                toast.info("Post criado com sucesso!");
            }

            // Limpa o formulário e atualiza a lista em qualquer caso de sucesso
            setNewPostContent("");
            setNewPostImageUrl("");
            refetchPosts();
        }
    }, [createPostSuccess, createPostResponseData, refetchPosts]); // Adicione a nova dependência
    ...
    return (
        <Container maxWidth="md" className="py-8 mt-16 min-h-screen">
            ...
            <Box className="flex flex-col space-y-4">
                ...
                {postsError && <Alert severity="error" className="w-full mb-4">{(postsError as any)?.data?.error || t("errorLoadingPosts")}</Alert>}
